...
class Workspace:
    def __init__(self, cfg):
        ...
        assert cfg.agent.name in ["rnd", "icm", "ddpg"]

        if cfg.agent.name == "ddpg":
            if cfg.agent.sac:
                cfg.experiment += "_sac"
            else:
                cfg.experiment += "_ddpg"
        ...
        # create logger
        if cfg.use_wandb:
            project = "dusdi_" + cfg.domain + "_downstream_" + cfg.ds_task # cfg.agent.name
            self.run = wandb.init(project=project, group=cfg.experiment, name=cfg.experiment)
        ...
        # create envs
        make_envs_func = make_ds_envs(cfg, None, self.device)
        self.train_env = make_envs_func()
        self.eval_env = make_envs_func()
